...
from sb3_py2.buffers import DictReplayBuffer, ReplayBuffer, HerReplayBuffer
from sb3_py2.type_aliases import TrainFreq, TrainFrequencyUnit
...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        if sde_support:
            self.policy_kwargs[u"use_sde"] = self.use_sde
        # For gSDE only
    ...
    def _setup_model(self):
        ...
        if self.replay_buffer_class is None:
            ...
            assert self.env is not None, u"You must pass an environment when using `HerReplayBuffer`"

            # If using offline sampling, we need a classic replay buffer too
            if self.replay_buffer_kwargs.get(u"online_sampling", True):
                replay_buffer = None
            else:
            ...
            replay_buffer_kwargs = {}
            replay_buffer_kwargs.update(self.replay_buffer_kwargs)
        ...
        policy_kwargs = {}
        policy_kwargs.update(self.policy_kwargs)
