...
def resume_phases(incomplete_data: DataFrame) -> None:
    ...
    if conf.wandb.experiment_tag == str(tag):
            conf.wandb.experiment_tag = [tag, resume_tag]
    elif conf.wandb.experiment_tag == Iterable(tags):
...
def phase(conf: DictConfig, phase_index: int, timestamp: str, ancestor = None) -> None:
...
def schedule_runs(
    ...
    return [results[i][0] for i in range(len(fn_args))]
