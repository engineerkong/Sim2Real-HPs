...
class SimplexInterpolator:
    ...
    def get_oob_mask(self, x: NDArray[Any], y_lower: NDArray[Any], y_upper: NDArray[Any]) -> NDArray[np.bool_]:
        ...
        y_si = self(x)
        return (y_si < y_lower - ATOL) or (y_si > y_upper + ATOL)
    ...
    def replace_simplices(self, points: NDArray[Any], side: str) -> Any:
        ...
        border_corner_inds = set(np.where(np.any((self.points[:, 0:-1] == 0) or (self.points[:, 0:-1] == 1), axis=1))[0])

        # corners that should be included in the new hull:
        surrounding_corner_inds = touched_corner_inds & (incident_corner_inds or border_corner_inds)
    ...
    def _build_simplex_function(self, simplex_inds: NDArray[Any]):
